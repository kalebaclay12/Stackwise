...
export default function EditStackModal({ stack, onClose }: EditStackModalProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // If user is on free tier, force autoAllocate to false
      const finalAutoAllocate = canUseAutoAllocation ? autoAllocate : false;
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Edit3 className="w-6 h-6 text-primary-600 dark:text-primary-400" />
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Edit Stack</h2>
          </div>
